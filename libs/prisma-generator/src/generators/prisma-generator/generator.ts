  ...
  const projectJsonContent = {
    ...
    "targets": {
      "deploy": {
        ...
        "options": {
          "command": `kubectl apply -f database-${name}.yaml`,
      ...
      "delete": {
        "executor": "nx:run-commands",
        "options": {
          "command": `kubectl delete -f database-${name}.yaml`,
          "cwd": infrastructurePath
        }
      },
      "generate": {
        "executor": "nx:run-commands",
        "options": {
          "command": `npx prisma generate --schema="./libs/prisma-clients/${name}/prisma/schema.prisma"`
        }
      },
      "migrate": {
        "executor": "nx:run-commands",
        "options": {
          "command": `npx prisma db push --schema="./libs/prisma-clients/${name}/prisma/schema.prisma"`
        }
      }
    }
  };
